...
const { saveItemsToDatabase, saveUserToDatabase, getUserFromDatabase } = require('./database');
  ...
  req.on('data', (chunk) => {
    ...
    const { username, password } = qs.parse(data);
      ...
      // 회원 정보 저장
        ...
        saveUserToDatabase(users[username]);
...
app.post('/login', (req, res) => {
  ...
  req.on('data', (chunk) => {
    ...
    .then((user) => {
       // 검색된 사용자 정보를 이용하여 작업 수행
       // 로그인 실패 처리
       if (!users[username] || hashedPassword !== user.passwd) {
